...
def pairwise_distances(xq, xb, metric=METRIC_L2, metric_arg=0):
    ...
    if metric == METRIC_L2:
        ...
        pairwise_extra_distances(d, nq, swig_ptr(xq), nb, swig_ptr(xb), metric,
                                 metric_arg, swig_ptr(dis))
    return dis
...
def eval_intersection(I1, I2):
    ...
    ninter = 0
    for i in range(n):
        ninter += ranklist_intersection_size(k1, swig_ptr(I1[i]), k2,
                                             swig_ptr(I2[i]))
    return ninter
...
def bucket_sort(tab, nbucket=None, nt=0):
    ...
    bucket_sort_c(tab.size, faiss.swig_ptr(tab.view('uint64')), nbucket,
                  faiss.swig_ptr(lims), faiss.swig_ptr(perm), nt)
    return lims, perm
...
def matrix_bucket_sort_inplace(tab, nbucket=None, nt=0):
    ...
    if nbucket is None:
        nbucket = int(tab.max() + 1)
    lims = np.empty(nbucket + 1, dtype='int64')
    matrix_bucket_sort_inplace_c(nrow, ncol, faiss.swig_ptr(tab), nbucket,
                                 faiss.swig_ptr(lims), nt)
    return lims
...
class ResultHeap:
    ...
    def add_result_subset(self, subset, D, I):
        ...
        assert (I.ndim == 2 and D.shape == I.shape
                or I.ndim == 1 and I.shape == (kd, ))
        D = np.ascontiguousarray(D, dtype='float32')
        I = np.ascontiguousarray(I, dtype='int64')
        subset = np.ascontiguousarray(subset, dtype='int64')
        id_stride = 0 if I.ndim == 1 else kd
        self.heaps.addn_query_subset_with_ids(nsubset, swig_ptr(subset), kd,
...
def merge_knn_results(Dall, Iall, keep_max=False):
    ...
    func(n, k, nshard, swig_ptr(Dall), swig_ptr(Iall), swig_ptr(Dnew),
         swig_ptr(Inew))
    return Dnew, Inew
...
class MapInt64ToInt64:
    ...
    def __init__(self, capacity):
        ...
        faiss.hashtable_int64_to_int64_init(self.log2_capacity,
                                            swig_ptr(self.tab))

    def add(self, keys, vals):
        n, = keys.shape
        assert vals.shape == (n, )
        faiss.hashtable_int64_to_int64_add(self.log2_capacity,
                                           swig_ptr(self.tab), n,
                                           swig_ptr(keys), swig_ptr(vals))

    def lookup(self, keys):
        n, = keys.shape
        vals = np.empty((n, ), dtype='int64')
        faiss.hashtable_int64_to_int64_lookup(self.log2_capacity,
                                              swig_ptr(self.tab), n,
                                              swig_ptr(keys), swig_ptr(vals))
        return vals
...
def knn(xq, xb, k, metric=METRIC_L2, metric_arg=0.0):
    ...
    if metric == METRIC_L2:
        knn_L2sqr(swig_ptr(xq), swig_ptr(xb), d, nq, nb, k, swig_ptr(D),
                  swig_ptr(I))
    elif metric == METRIC_INNER_PRODUCT:
        knn_inner_product(swig_ptr(xq), swig_ptr(xb), d, nq, nb, k,
                          swig_ptr(D), swig_ptr(I))
    else:
        knn_extra_metrics(swig_ptr(xq), swig_ptr(xb), d, nq, nb, metric,
                          metric_arg, k, swig_ptr(D), swig_ptr(I))
...
def knn_hamming(xq, xb, k, variant="hc"):
    ...
    if variant == "hc":
        ...
        faiss.hammings_knn_hc(heap, faiss.swig_ptr(xq), faiss.swig_ptr(xb), nb,
                              d, 1)
    elif variant == "mc":
        faiss.hammings_knn_mc(faiss.swig_ptr(xq), faiss.swig_ptr(xb), nq, nb,
                              k, d, faiss.swig_ptr(D), faiss.swig_ptr(I))
    else:
    ...
    def train(self, x, weights=None, init_centroids=None):
        ...
        if self.cp.__class__ == ClusteringParameters:
            ...
            if init_centroids is not None:
                ...
                assert d2 == d
                faiss.copy_array_to_vector(init_centroids.ravel(),
                                           clus.centroids)
...
def pack_bitstrings(a, nbit):
    ...
    if is_sequence(nbit):
        ...
        pack_bitstrings_c(n, M, swig_ptr(nbit), swig_ptr(a), swig_ptr(b),
                          code_size)
    else:
...
def unpack_bitstrings(b, M_or_nbits, nbit=None):
    ...
    if nbit is None:
        ...
        unpack_bitstrings_c(n, M, swig_ptr(nbit), swig_ptr(b), code_size,
                            swig_ptr(a))
    else:
        M = M_or_nbits
