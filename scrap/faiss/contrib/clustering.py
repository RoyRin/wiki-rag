...
def sparse_assign_to_dense_blocks(xq,
                                  xb,
                                  xq_norms=None,
                                  xb_norms=None,
                                  qbs=16384,
...
class DatasetAssignSparse(DatasetAssign):
    ...
    def perform_search(self, centroids):
        return sparse_assign_to_dense_blocks(self.x,
                                             centroids,
                                             xq_norms=self.squared_norms)
    ...
    def assign_to(self, centroids, weights=None):
        ...
        m = scipy.sparse.csc_matrix((weights, I, np.arange(n + 1)),
                                    shape=(nc, n))
...
def kmeans(k,
    ...
    for i in range(niter):
        ...
        log(("  Iteration %d (%.2f s, search %.2f s): "
            ...
            (i, s["time"], s["time_search"], err, s["imbalance_factor"],
             nsplit))
