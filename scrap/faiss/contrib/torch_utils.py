...
def swig_ptr_from_UInt8Tensor(x):
    ...
    return faiss.cast_integer_to_uint8_ptr(x.untyped_storage().data_ptr() +
                                           x.storage_offset())
...
def swig_ptr_from_HalfTensor(x):
    ...
    return faiss.cast_integer_to_void_ptr(x.untyped_storage().data_ptr() +
                                          x.storage_offset() * 2)
...
def swig_ptr_from_FloatTensor(x):
    ...
    return faiss.cast_integer_to_float_ptr(x.untyped_storage().data_ptr() +
                                           x.storage_offset() * 4)
...
def swig_ptr_from_IntTensor(x):
    ...
    return faiss.cast_integer_to_int_ptr(x.untyped_storage().data_ptr() +
                                         x.storage_offset() * 4)
...
def swig_ptr_from_IndicesTensor(x):
    ...
    return faiss.cast_integer_to_idx_t_ptr(x.untyped_storage().data_ptr() +
                                           x.storage_offset() * 8)
...
def torch_replace_method(the_class,
                         name,
                         replacement,
                         ignore_missing=False,
                         ignore_no_base=False):
    try:
...
def handle_torch_Index(the_class):
    ...
    def torch_replacement_add(self, x):
        ...
        if x.is_cuda:
            assert hasattr(self,
                           'getDevice'), 'GPU tensor on CPU index not allowed'
    ...
    def torch_replacement_search_and_reconstruct(self,
                                                 x,
                                                 k,
                                                 D=None,
                                                 I=None,
    ...
    def torch_replacement_search_preassigned(self,
                                             x,
                                             k,
                                             Iq,
                                             Dq,
        ...
        if x.is_cuda:
            ...
            # On the GPU, use proper stream ordering
            with using_stream(self.getResources()):
                self.search_preassigned_c(n, x_ptr, k, Iq_ptr, Dq_ptr, D_ptr,
                                          I_ptr, False)
        else:
            # CPU torch
            self.search_preassigned_c(n, x_ptr, k, Iq_ptr, Dq_ptr, D_ptr,
    ...
    def torch_replacement_remove_ids(self, x):
        # Not yet implemented
        assert type(
            x) is not torch.Tensor, 'remove_ids not yet implemented for torch'
    ...
    torch_replace_method(the_class,
                         'update_vectors',
...
def torch_replacement_knn(xq, xb, k, metric=faiss.METRIC_L2, metric_arg=0):
    ...
    if metric == faiss.METRIC_L2:
        ...
        faiss.knn_extra_metrics(xq_ptr, xb_ptr, d, nq, nb, metric, metric_arg,
                                k, D_ptr, I_ptr)
...
torch_replace_method(faiss_module, 'knn_gpu', torch_replacement_knn_gpu, True,
                     True)


# allows torch tensor usage with bfKnn for all pairwise distances
def torch_replacement_pairwise_distance_gpu(res,
                                            xq,
                                            xb,
                                            D=None,
                                            metric=faiss.METRIC_L2,
