...
class BigBatchSearcher:
    ...
    def report(self, l):
        if self.verbose == 1 or (self.verbose == 2 and
                                 (l > 1000
                                  and time.time() < self.t_display + 1.0)):
            return
        t = time.time() - self.t0
        print(
...
class BlockComputer:
    ...
    def __init__(self,
        ...
        if index.__class__ == faiss.IndexIVFFlat:
            ...
            index_help = faiss.IndexPQ(index.d, index.pq.M, index.pq.nbits,
                                       index.metric_type)
            ...
            index_help = faiss.IndexScalarQuantizer(index.d, index.sq.qtype,
                                                    index.metric_type)
...
def big_batch_search(index,
    ...
    mem_res = len(xq) * k * (np.dtype('int64').itemsize +
    ...
    bbs = BigBatchSearcher(index,
                           xq,
                           k,
                           verbose=verbose,
                           use_float16=use_float16)

    comp = BlockComputer(index,
                         method=method,
    ...
    if threaded == 0:
        ...
        for l in range(start_list, end_list):
            bbs.report(l)
            prefetched_bucket_a = pool.apply_async(add_results_and_prefetch,
                                                   (to_add, l + 1))
        ...
        def compute_task(task_id, output_queue, input_queue):
            try:
                ...
                while True:
                    ...
                    t0 = time.time()
                    output_queue.put((centroid, t_wait_in, t_wait_out,
                                      t_compute, q_subset, D, list_ids, I))
