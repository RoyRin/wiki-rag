...
def range_search_gpu(xq, r2, index_gpu, index_cpu, gpu_k=1024):
    ...
    CombinerRangeKNN = (faiss.CombinerRangeKNNint16
                        if is_binary_index else faiss.CombinerRangeKNNfloat)
...
def apply_maxres(res_batches, target_nres, keep_max=False):
    ...
    for i, (nres, dis, ids) in enumerate(res_batches):
        nres, dis, ids = threshold_radius_nres(nres,
                                               dis,
                                               ids,
                                               radius,
                                               keep_max=keep_max)
...
def range_search_max_results(index,
                             query_iterator,
                             radius,
                             max_results=None,
                             min_results=None,
