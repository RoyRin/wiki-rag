...
__version__ = "%d.%d.%d" % (FAISS_VERSION_MAJOR, FAISS_VERSION_MINOR,
                            FAISS_VERSION_PATCH)
...
class_wrappers.handle_IDSelectorSubset(IDSelectorBitmap,
                                       class_owns=False,
                                       force_int64=False)
...
try:
    ...
    logger.info(
        "Failed to load GPU Faiss: %s. Will not load constructor refs for GPU indexes. This is only an error if you're trying to use GPU Faiss."
        % e.args[0])
...
def search_with_parameters(index, x, k, params=None, output_stats=False):
    ...
    search_with_parameters_c(index, n, swig_ptr(x), k, swig_ptr(distances),
                             swig_ptr(labels), params, swig_ptr(nb_dis),
...
def range_search_with_parameters(index,
                                 x,
                                 radius,
                                 params=None,
                                 output_stats=False):
    ...
    range_search_with_parameters_c(index, n, swig_ptr(x), radius, res, params,
                                   swig_ptr(nb_dis), swig_ptr(ms_per_stage))
